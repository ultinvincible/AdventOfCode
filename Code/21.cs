...
namespace Advent_of_Code
{
    class Dice : AoCDay
    {
        ...
        {
            int[] state = new int[]{(int)char.GetNumericValue(input[0][^1]),
                                    (int)char.GetNumericValue(input[1][^1]) , 0 , 0 };
            ...
            Console.WriteLine(Math.Min(state[2], state[3]) * timesRolled);
            ...
            List<(int pos, int score, long count)>[] players = new List<(int, int, long)>[] {
                new() { ((int)char.GetNumericValue(input[0][^1]), 0, 1) },
                new() { ((int)char.GetNumericValue(input[1][^1]), 0, 1) } };
            plrTurn = 0;
            ...
            Console.WriteLine(Math.Max(wins[0], wins[1]));
        }
    }
}
